# image-build-csi-resizer

This repo builds hardened, statically-linked Go binaries from
[kubernetes-csi/external-resizer](https://github.com/kubernetes-csi/external-resizer) and packages them in a minimal
...
- `rancher/hardened-csi-resizer` — CSI Resizer Sidecar

## Building locally

```sh
make build-image-build-image-csi-resizer    # build for the host architecture
...
- the upstream `external-resizer` version (`TAG`), and
